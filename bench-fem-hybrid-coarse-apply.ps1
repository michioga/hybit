param(
    ...
    [string]$CsvPath = 'hybit-hybrid-coarse-apply.csv'
)
...
function Convert-BenchResult {
    param([string]$Policy, [int]$Repeat, [string[]]$Lines)
...
Write-Host '== HyBIT coarse-apply repeated A/B =='
Write-Host "Matrix: $Matrix"
if ($Rhs) { Write-Host "RHS: $Rhs" }
Write-Host "Coarse target: $CoarseTarget"
Write-Host "Repeats: $Repeats"
Write-Host 'Policy order alternates factor/inverse between repeats.'

$results = [System.Collections.Generic.List[object]]::new()
for ($repeat = 1; $repeat -le $Repeats; $repeat++) {
    $policies = if (($repeat % 2) -eq 1) { @('factor','inverse') } else { @('inverse','factor') }
    foreach ($policy in $policies) {
        $lines = Invoke-HybitBench -Label "repeat $repeat/$Repeats : coarse apply = $policy" -Policy $policy
        $results.Add((Convert-BenchResult -Policy $policy -Repeat $repeat -Lines $lines))
    }
}
...
foreach ($policy in @('factor','inverse')) {
    $runs = @($results | Where-Object { $_.Policy -eq $policy })
...
Write-Host '== coarse-apply A/B complete =='
