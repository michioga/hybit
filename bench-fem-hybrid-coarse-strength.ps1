param(
    ...
    [string]$CsvPath = 'hybit-hybrid-coarse-strength.csv'
)
...
function Invoke-HybitBench {
    param(
        ...
        [Parameter(Mandatory=$true)][ValidateSet('graph','strong-graph')][string]$Aggregation
    )
    Write-Host ''
    Write-Host '============================================================'
    Write-Host " $Label"
...
Write-Host '== HyBIT graph strength-of-connection repeated A/B =='
Write-Host "Matrix: $Matrix"
if ($Rhs) { Write-Host "RHS: $Rhs" }
Write-Host "Coarse target: $CoarseTarget"
Write-Host "Coarse apply: $CoarseApply"
Write-Host "Repeats: $Repeats"
Write-Host 'Aggregation order alternates graph/strong-graph between repeats.'

$results = [System.Collections.Generic.List[object]]::new()
for ($repeat = 1; $repeat -le $Repeats; $repeat++) {
    $aggregations = if (($repeat % 2) -eq 1) { @('graph','strong-graph') } else { @('strong-graph','graph') }
...
foreach ($aggregation in @('graph','strong-graph')) {
...
$strong = $summary | Where-Object { $_.Aggregation -eq 'strong-graph' }
if ($graph -and $strong) {
    $iterGain = 100.0 * ($graph.MedianIterations - $strong.MedianIterations) / $graph.MedianIterations
    $solverGain = 100.0 * ($graph.MedianSolverMs - $strong.MedianSolverMs) / $graph.MedianSolverMs
    $wallGain = 100.0 * ($graph.MedianWallMs - $strong.MedianWallMs) / $graph.MedianWallMs
    Write-Host ('StrongGraph vs Graph: iteration gain {0:F2}%, solver gain {1:F2}%, wall gain {2:F2}%.' -f $iterGain, $solverGain, $wallGain)
}

$results | Export-Csv -Path $CsvPath -NoTypeInformation -Encoding UTF8
Write-Host "CSV: $CsvPath"
Write-Host '== graph strength-of-connection A/B complete =='
