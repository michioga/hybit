param(
    ...
    [string]$CsvPath = 'hybit-hybrid-coarse-transfer.csv'
)
...
function Invoke-HybitBench {
    param(
        ...
        [Parameter(Mandatory=$true)][ValidateSet('serial','parallel')][string]$Transfer
    )
    Write-Host ''
    Write-Host '============================================================'
    Write-Host " $Label"
    ...
    $cargoArgs += @('--coarse-basis', 'smoothed', '--coarse-transfer', $Transfer)
...
function Convert-BenchResult {
    param([string]$Transfer, [int]$Repeat, [string[]]$Lines)
...
Write-Host '== HyBIT smoothed coarse-transfer repeated A/B =='
...
Write-Host 'Transfer order alternates serial/parallel between repeats.'

$results = [System.Collections.Generic.List[object]]::new()
for ($repeat = 1; $repeat -le $Repeats; $repeat++) {
    $transfers = if (($repeat % 2) -eq 1) { @('serial','parallel') } else { @('parallel','serial') }
    foreach ($transfer in $transfers) {
        $lines = Invoke-HybitBench -Label "repeat $repeat/$Repeats : coarse transfer = $transfer" -Transfer $transfer
        $results.Add((Convert-BenchResult -Transfer $transfer -Repeat $repeat -Lines $lines))
    }
}
...
foreach ($transfer in @('serial','parallel')) {
    $runs = @($results | Where-Object { $_.Transfer -eq $transfer })
...
$serial = $summary | Where-Object { $_.Transfer -eq 'serial' }
$parallel = $summary | Where-Object { $_.Transfer -eq 'parallel' }
if ($serial -and $parallel) {
    $iterDelta = 100.0 * ($serial.MedianIterations - $parallel.MedianIterations) / $serial.MedianIterations
    $solverGain = 100.0 * ($serial.MedianSolverMs - $parallel.MedianSolverMs) / $serial.MedianSolverMs
    $wallGain = 100.0 * ($serial.MedianWallMs - $parallel.MedianWallMs) / $serial.MedianWallMs
    $memoryDelta = $parallel.MedianCoarseMiB - $serial.MedianCoarseMiB
    Write-Host ('Parallel vs serial transfer: iteration delta {0:F2}%, solver gain {1:F2}%, wall gain {2:F2}%, coarse-memory delta {3:F2} MiB.' -f $iterDelta, $solverGain, $wallGain, $memoryDelta)
}

$results | Export-Csv -Path $CsvPath -NoTypeInformation -Encoding UTF8
Write-Host "CSV: $CsvPath"
Write-Host '== smoothed coarse-transfer A/B complete =='
