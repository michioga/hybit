param(
    ...
    [string]$CsvPath = 'hybit-hybrid-smoothed-coarse-sweep.csv',
    [string]$SummaryCsvPath = 'hybit-hybrid-smoothed-coarse-sweep-summary.csv'
)
...
Write-Host '== HyBIT smoothed+parallel coarse-dimension repeated sweep =='
...
for ($repeat = 1; $repeat -le $Repeats; $repeat++) {
    ...
    foreach ($target in $orderedTargets) {
        $lines = Invoke-HybitBench -Label "repeat $repeat/$Repeats : smoothed+parallel coarse target = $target" -Target $target
...
if ($eligible.Count -gt 0) {
    $best = $eligible[0]
    Write-Host ''
    Write-Host ('Best all-converged smoothed+parallel target: {0} (actual {1}, {2}), median wall {3:F2} ms, solver {4:F2} ms, {5:F0} iters, coarse {6:F2} MiB.' -f `
...
if ($baseline -and $eligible.Count -gt 0) {
    ...
    Write-Host ('Best vs smoothed+parallel target 1792: wall gain {0:F2}%, coarse-memory reduction {1:F2}%.' -f $wallGain, $memoryGain)
}
...
Write-Host '== smoothed+parallel coarse-dimension sweep complete =='
