param(
    ...
    [string]$CsvPath = 'hybit-hybrid-coarse-aggregation.csv'
)
...
function Invoke-HybitBench {
    param(
        ...
        [Parameter(Mandatory=$true)][ValidateSet('contiguous','graph')][string]$Aggregation
    )
    Write-Host ''
    Write-Host '============================================================'
    Write-Host " $Label"
    ...
    $cargoArgs += @('--coarse-aggregation', $Aggregation)
...
function Convert-BenchResult {
    param([string]$Aggregation, [int]$Repeat, [string[]]$Lines)
...
Write-Host '== HyBIT generic coarse aggregation repeated A/B =='
Write-Host "Matrix: $Matrix"
if ($Rhs) { Write-Host "RHS: $Rhs" }
Write-Host "Coarse target: $CoarseTarget"
Write-Host "Coarse apply: $CoarseApply"
Write-Host "Repeats: $Repeats"
Write-Host 'Aggregation order alternates contiguous/graph between repeats.'

$results = [System.Collections.Generic.List[object]]::new()
for ($repeat = 1; $repeat -le $Repeats; $repeat++) {
    $aggregations = if (($repeat % 2) -eq 1) { @('contiguous','graph') } else { @('graph','contiguous') }
    foreach ($aggregation in $aggregations) {
        $lines = Invoke-HybitBench -Label "repeat $repeat/$Repeats : coarse aggregation = $aggregation" -Aggregation $aggregation
        $results.Add((Convert-BenchResult -Aggregation $aggregation -Repeat $repeat -Lines $lines))
    }
}
...
foreach ($aggregation in @('contiguous','graph')) {
    $runs = @($results | Where-Object { $_.Aggregation -eq $aggregation })
...
$contiguous = $summary | Where-Object { $_.Aggregation -eq 'contiguous' }
$graph = $summary | Where-Object { $_.Aggregation -eq 'graph' }
if ($contiguous -and $graph) {
    $iterGain = 100.0 * ($contiguous.MedianIterations - $graph.MedianIterations) / $contiguous.MedianIterations
    $solverGain = 100.0 * ($contiguous.MedianSolverMs - $graph.MedianSolverMs) / $contiguous.MedianSolverMs
    $wallGain = 100.0 * ($contiguous.MedianWallMs - $graph.MedianWallMs) / $contiguous.MedianWallMs
    Write-Host ('Graph vs contiguous: iteration gain {0:F2}%, solver gain {1:F2}%, wall gain {2:F2}%.' -f $iterGain, $solverGain, $wallGain)
}

$results | Export-Csv -Path $CsvPath -NoTypeInformation -Encoding UTF8
Write-Host "CSV: $CsvPath"
Write-Host '== generic coarse aggregation A/B complete =='
