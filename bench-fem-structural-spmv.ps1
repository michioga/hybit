...
try {
    foreach ($threadCount in $Threads) {
        ...
        $cargoArgs = @(
            "run", "--release", "-p", "hybit", "--example", "fem_structural_spmv", "--",
            "--matrix", $Matrix,
            "--coords", $Coordinates,
        ...
        if ($LASTEXITCODE -ne 0) {
            throw "parallel CSR benchmark failed for $threadCount threads with exit code $LASTEXITCODE"
        }
    }
}
finally {
...
Write-Host ""
Write-Host "== serial vs parallel CSR sweep complete =="
