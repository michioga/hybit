param(
    ...
    [string]$CsvPath = 'hybit-hybrid-escalation-depth.csv',
    [string]$SummaryCsvPath = 'hybit-hybrid-escalation-depth-summary.csv'
)
...
function Invoke-HybitBench {
    ...
    $cargoArgs += @('--max-escalations', $Depth.ToString($Invariant))
...
function Convert-BenchResult {
    param([int]$Depth, [int]$Repeat, [string[]]$Lines)
...
for ($repeat = 1; $repeat -le $Repeats; $repeat++) {
    ...
    foreach ($index in $order) {
        $depth = $depths[$index]
        $label = "repeat $repeat/$Repeats : max escalations = $depth"
        $lines = Invoke-HybitBench -Label $label -Depth $depth
        $results.Add((Convert-BenchResult -Depth $depth -Repeat $repeat -Lines $lines))
    }
}
...
foreach ($depth in $depths) {
    $runs = @($results | Where-Object { $_.MaxEscalations -eq $depth })
    ...
    $summary.Add([PSCustomObject]@{
        ...
        MedianActualEscalations = Get-Median -Values @($runs | ForEach-Object { [double]$_.ActualEscalations })
...
$summary | ForEach-Object {
    [PSCustomObject]@{
        ...
        Esc = $_.MedianActualEscalations.ToString('0', $Invariant)
...
$baseline = $summary | Where-Object { $_.MaxEscalations -eq 3 } | Select-Object -First 1
Write-Host ''
Write-Host '== relative to max escalations 3 =='
$summary | ForEach-Object {
    ...
    [PSCustomObject]@{
        ...
        RegionDelta = [int]($_.MedianLocalRegions - $baseline.MedianLocalRegions)
...
if ($allConverged.Count -gt 0) {
    ...
    Write-Host ''
    Write-Host ("Fastest all-converged escalation depth: {0}, median wall {1:F2} ms, solver {2:F2} ms, {3:F0} iters, {4:F0} actual escalations, {5:F0} regions." -f $best.MaxEscalations, $best.MedianWallMs, $best.MedianSolverMs, $best.MedianIterations, $best.MedianActualEscalations, $best.MedianLocalRegions)
}
...
Write-Host '== smoothed+parallel escalation-depth comparison complete =='
