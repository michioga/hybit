...
use hybit::{
    ...
    BalancedRigidBodyTwoLevelBlockJacobiPreconditioner, Csr32Matrix, PcgWorkspace,
...
fn main() -> Result<(), Box<dyn Error>> {
    let args = Args::parse()?;
    println!(
        "HyBIT {} additive vs balanced structural FEM benchmark",
    ...
    if !profile.square || !profile.full_diagonal || !profile.positive_diagonal {
        return Err(
            "balanced structural PCG requires a square matrix with a complete positive diagonal"
                .into(),
        );
    }
    ...
    println!(
        ...
        additive.min_aggregate_nodes(),
        additive.max_aggregate_nodes()
    );
    println!("coarse dimension   : {}", additive.coarse_dimension());
    println!(
        "prec storage       : {:.3} MiB",
        mib(additive.factor_bytes())
    );
    ...
    let balanced = match args.aggregation {
        RigidBodyAggregation::Graph => {
            BalancedRigidBodyTwoLevelBlockJacobiPreconditioner::from_csr32_graph(
                &matrix,
                &coordinates,
                aggregate_nodes,
            )?
        ...
        RigidBodyAggregation::Contiguous => {
            BalancedRigidBodyTwoLevelBlockJacobiPreconditioner::from_csr32(
                &matrix,
                &coordinates,
                aggregate_nodes,
            )?
    ...
    println!(
        "aggregate min/max  : {} / {} nodes",
        balanced.min_aggregate_nodes(),
        balanced.max_aggregate_nodes()
    );
    println!("coarse dimension   : {}", balanced.coarse_dimension());
    println!(
        "factor storage     : {:.3} MiB",
        mib(balanced.factor_bytes())
    );
    println!(
        "balance workspace  : {:.3} MiB",
        mib(balanced.workspace_bytes())
    );
    ...
    println!(
        "reported residual  : {:.6e}",
        out_bal.final_residual / norm2(&b).max(f64::MIN_POSITIVE)
    );
    println!("verified residual  : {:.6e}", bal_verified);
    println!("solve time         : {:.3} ms", balanced_solve * 1.0e3);
    println!(
        "total setup+solve  : {:.3} ms",
        (balanced_setup + balanced_solve) * 1.0e3
    );

    println!();
    println!("Comparison");
    println!(
        "iteration ratio    : {:.3} (balanced / additive)",
        out_bal.iterations as f64 / (out_add.iterations.max(1) as f64)
    );
    println!(
        "solve-time ratio   : {:.3} (balanced / additive)",
        balanced_solve / additive_solve.max(f64::MIN_POSITIVE)
    );
    println!(
        "total-time ratio   : {:.3} (balanced / additive)",
        (balanced_setup + balanced_solve)
            / (additive_setup + additive_solve).max(f64::MIN_POSITIVE)
    );
