use hybit::{
    pcg, Csr32Matrix, HybitSolver, JacobiPreconditioner, PreconditionerKind, SolveStatus,
    SolverOptions,
};

fn easy_plus_hard_block(
    easy: usize,
    hard: usize,
) -> Result<Csr32Matrix, Box<dyn std::error::Error>> {
    let n = easy + hard;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "HyBIT {} selective-direct / Schwarz demo",
        env!("CARGO_PKG_VERSION")
    );
    println!(
        "plain Jacobi-PCG : status={:?}, iterations={}, residual={:.3e}",
        plain.status, plain.iterations, plain.final_residual
    );
    println!(
        "HyBIT auto       : status={:?}, iterations={}, residual={:.3e}",
        report.status, report.iterations, report.final_residual
    );
    println!("  escalations    : {}", report.escalations);
    println!(
        "  probe           : {} iterations -> {:.3e}",
        report.probe_iterations, report.probe_final_residual
    );
    ...
    println!(
        "  factor memory   : {:.3} KiB",
        report.local_factor_bytes as f64 / 1024.0
    );
