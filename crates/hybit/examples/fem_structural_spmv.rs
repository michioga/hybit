...
use hybit::{
    ...
    Csr32Matrix, LinearOperator, ParallelCsr32Operator, PcgWorkspace, RigidBodyAggregation,
...
fn main() -> Result<(), Box<dyn Error>> {
    let args = Args::parse()?;
    println!(
        "HyBIT {} serial vs parallel CSR structural benchmark",
    ...
    println!(
        "serial / call      : {:.3} ms",
        ms_per(serial_spmv, args.kernel_repeats)
    );
    println!(
        "parallel / call    : {:.3} ms",
        ms_per(parallel_spmv, args.kernel_repeats)
    );
    println!(
        "SpMV speedup       : {:.3}x",
        serial_spmv / parallel_spmv.max(f64::MIN_POSITIVE)
    );
    ...
    let parallel_out =
        pcg_with_workspace(&parallel, &preconditioner, &b, &mut xp, options, &mut wp)?;
    ...
    println!("Comparison");
    println!(
        "iteration ratio    : {:.3} (parallel / serial)",
        parallel_out.iterations as f64 / serial_out.iterations.max(1) as f64
    );
    println!(
        "solve-time ratio   : {:.3} (parallel / serial)",
