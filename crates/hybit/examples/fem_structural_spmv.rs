...
use hybit::{
    ...
    recommend_rigid_body_aggregate_nodes, Csr32Matrix, LinearOperator, ParallelCsr32Operator,
...
fn print_usage() {
    println!("HyBIT structural serial vs parallel CSR SpMV benchmark");
    println!("Usage: fem_structural_spmv --matrix K.mtx [--coords K.coords] [--rhs b.txt] [--tol 1e-8] [--max-iters 3000] [--target-coarse-dim 1536] [--aggregation graph|contiguous] [--kernel-repeats 20]");
    println!("Set RAYON_NUM_THREADS before launch to control parallel CSR thread count.");
}
...
fn ms_per(seconds: f64, repeats: usize) -> f64 { seconds * 1.0e3 / repeats as f64 }
...
fn main() -> Result<(), Box<dyn Error>> {
    let args = Args::parse()?;
    println!("HyBIT {} serial vs parallel CSR structural benchmark", env!("CARGO_PKG_VERSION"));
    ...
    let parallel = ParallelCsr32Operator::new(&matrix);
    ...
    println!("Rayon threads      : {}", parallel.rayon_threads());

    let mut ys = vec![0.0; matrix.nrows()];
    let mut yp = vec![0.0; matrix.nrows()];
    matrix.apply(&b, &mut ys)?;
    parallel.apply(&b, &mut yp)?;
    if ys != yp {
        return Err("serial and parallel CSR kernels produced different results".into());
    }

    let serial_start = Instant::now();
    for _ in 0..args.kernel_repeats { matrix.apply(&b, &mut ys)?; }
    let serial_spmv = serial_start.elapsed().as_secs_f64();

    let parallel_start = Instant::now();
    for _ in 0..args.kernel_repeats { parallel.apply(&b, &mut yp)?; }
    let parallel_spmv = parallel_start.elapsed().as_secs_f64();

    println!();
    println!("CSR SpMV microbenchmark");
    println!("serial / call      : {:.3} ms", ms_per(serial_spmv, args.kernel_repeats));
    println!("parallel / call    : {:.3} ms", ms_per(parallel_spmv, args.kernel_repeats));
    println!("SpMV speedup       : {:.3}x", serial_spmv / parallel_spmv.max(f64::MIN_POSITIVE));
    ...
    let serial_out = pcg_with_workspace(&matrix, &preconditioner, &b, &mut xs, options, &mut ws)?;
    ...
    let parallel_out = pcg_with_workspace(&parallel, &preconditioner, &b, &mut xp, options, &mut wp)?;
    let parallel_solve = parallel_solve_start.elapsed().as_secs_f64();
    let parallel_verified = verified_relative_residual(&matrix, &b, &xp)?;

    println!();
    println!("[1/2] Serial CSR PCG");
    println!("status             : {:?}", serial_out.status);
    println!("iterations         : {}", serial_out.iterations);
    println!("reported residual  : {:.6e}", serial_out.final_residual / norm2(&b).max(f64::MIN_POSITIVE));
    println!("verified residual  : {:.6e}", serial_verified);
    println!("solve time         : {:.3} ms", serial_solve * 1.0e3);

    println!();
    println!("[2/2] Parallel CSR PCG");
    println!("status             : {:?}", parallel_out.status);
    println!("iterations         : {}", parallel_out.iterations);
    println!("reported residual  : {:.6e}", parallel_out.final_residual / norm2(&b).max(f64::MIN_POSITIVE));
    println!("verified residual  : {:.6e}", parallel_verified);
    println!("solve time         : {:.3} ms", parallel_solve * 1.0e3);

    println!();
    println!("Comparison");
    println!("iteration ratio    : {:.3} (parallel / serial)", parallel_out.iterations as f64 / serial_out.iterations.max(1) as f64);
    println!("solve-time ratio   : {:.3} (parallel / serial)", parallel_solve / serial_solve.max(f64::MIN_POSITIVE));
    println!("PCG speedup        : {:.3}x", serial_solve / parallel_solve.max(f64::MIN_POSITIVE));
