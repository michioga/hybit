...
fn print_usage() {
    ...
    println!("Usage: fem_structural_prepared --matrix K.mtx [--coords K.coords] [--rhs b.txt] [--tol 1e-8] [--max-iters 3000] [--target-coarse-dim 1536] [--aggregation auto|contiguous|graph] [--spmv auto|serial|parallel] [--repeats 2]");
}
...
fn main() -> Result<(), Box<dyn Error>> {
    let args = Args::parse()?;
    println!("HyBIT {} prepared structural solve-many FEM benchmark", env!("CARGO_PKG_VERSION"));
    ...
    if !profile.square || !profile.full_diagonal || !profile.positive_diagonal {
        return Err("prepared structural PCG requires a square matrix with a complete positive diagonal".into());
    }
    ...
    println!("repeats            : {} (fresh zero initial guess each solve)", args.repeats);
    ...
    println!("Krylov workspace   : {:.3} MiB", mib(prepared.krylov_workspace_bytes()));
    println!("analysis once      : {:.3} ms", prepared.analysis_seconds() * 1.0e3);
    println!("prepare once       : {:.3} ms", prepared.prepare_seconds() * 1.0e3);
    ...
    for repetition in 1..=args.repeats {
        ...
        println!("analysis charged   : {:.3} ms", report.analysis_seconds * 1.0e3);
        println!("prepare charged    : {:.3} ms", report.prepare_seconds * 1.0e3);
        ...
        if repetition > 1 && !report.preconditioner_reused {
            return Err("subsequent prepared structural solve did not report preconditioner reuse".into());
        }
        if repetition > 1 && (report.analysis_seconds != 0.0 || report.prepare_seconds != 0.0) {
            return Err("subsequent prepared structural solve was charged reusable setup cost".into());
        }
    ...
    println!("one-time setup     : {:.3} ms", (prepared.analysis_seconds() + prepared.prepare_seconds()) * 1.0e3);
