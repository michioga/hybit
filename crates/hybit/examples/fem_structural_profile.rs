...
use hybit::{
    ...
    Csr32Matrix, LinearOperator, PcgWorkspace, RigidBodyAggregation,
...
fn ms_per(total_seconds: f64, repeats: usize) -> f64 {
    total_seconds * 1.0e3 / repeats as f64
}
...
fn main() -> Result<(), Box<dyn Error>> {
    let args = Args::parse()?;
    println!(
        "HyBIT {} structural kernel profile",
    ...
    if !profile.square || !profile.full_diagonal || !profile.positive_diagonal {
        return Err(
            "structural PCG profile requires a square matrix with a complete positive diagonal"
                .into(),
        );
    }
    ...
    println!(
        ...
        preconditioner.min_aggregate_nodes(),
        preconditioner.max_aggregate_nodes()
    );
    println!("coarse dimension   : {}", preconditioner.coarse_dimension());
    println!(
        "base factor        : {:.3} MiB",
        mib(preconditioner.base_factor_bytes())
    );
    println!(
        "coarse factor      : {:.3} MiB",
        mib(preconditioner.coarse_factor_bytes())
    );
    println!(
    ...
    let component_profile =
        preconditioner.profile_apply_components(&b, &mut z, args.kernel_repeats)?;
    ...
    println!();
    println!(
        "Kernel profile ({} repeats, normal solve path remains uninstrumented)",
        args.kernel_repeats
    );
    println!(
        "CSR SpMV / call     : {:.3} ms",
        ms_per(spmv_seconds, args.kernel_repeats)
    );
    println!(
        "3x3 block Jacobi    : {:.3} ms/apply",
        ms_per(base_seconds, args.kernel_repeats)
    );
    println!(
        "restriction Z^T r   : {:.3} ms/apply",
        ms_per(restriction_seconds, args.kernel_repeats)
    );
    println!(
        "coarse solve        : {:.3} ms/apply",
        ms_per(coarse_seconds, args.kernel_repeats)
    );
    println!(
        "prolongation Z e    : {:.3} ms/apply",
        ms_per(prolongation_seconds, args.kernel_repeats)
    );
    println!(
        "precond components  : {:.3} ms/apply",
        ms_per(precond_seconds, args.kernel_repeats)
    );
    ...
    let estimated_kernel_total_ms =
        (outcome.iterations + 1) as f64 * spmv_ms + outcome.iterations as f64 * precond_ms;
    ...
    println!(
        ...
        outcome.final_residual / norm2(&b).max(f64::MIN_POSITIVE)
    );
    ...
    println!(
        "estimated SpMV     : {:.3} ms total",
        (outcome.iterations + 1) as f64 * spmv_ms
    );
    println!(
        "estimated precond  : {:.3} ms total",
