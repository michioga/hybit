...
use hybit::{
    analyze_csr32, pcg, read_matrix_market, BlockJacobiPreconditioner, Csr32Matrix,
    SolverOptions,
};
...
impl Args {
    fn parse() -> Result<Self, Box<dyn Error>> {
        ...
        if block_size == 0 { return Err("--block-size must be > 0".into()); }
        Ok(Self { matrix, relative_tolerance, max_iterations, block_size })
    }
}
...
fn print_usage() {
    ...
    println!("Usage: fem_block_bench --matrix K.mtx [--tol 1e-8] [--max-iters 3000] [--block-size 3]");
}
...
fn main() -> Result<(), Box<dyn Error>> {
    let args = Args::parse()?;
    println!("HyBIT {} block-Jacobi FEM benchmark", env!("CARGO_PKG_VERSION"));
    ...
    if !profile.square || !profile.full_diagonal || !profile.positive_diagonal {
        return Err("block-Jacobi PCG requires a square matrix with a complete positive diagonal".into());
    }
    ...
    println!("factor storage     : {:.3} MiB", mib(precond.factor_bytes()));
