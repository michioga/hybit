...
use hybit::{
    ...
    recommend_rigid_body_aggregate_nodes, Csr32Matrix,
...
fn main() -> Result<(), Box<dyn Error>> {
    let args = Args::parse()?;
    println!("HyBIT {} serial vs parallel rigid-body preconditioner benchmark", env!("CARGO_PKG_VERSION"));
    ...
    if max_diff > 1.0e-11 * scale {
        return Err(format!("serial/parallel preconditioner mismatch: max diff={max_diff:e}, scale={scale:e}").into());
    }

    let serial_prec_start = Instant::now();
    for _ in 0..args.kernel_repeats { preconditioner.apply(&b, &mut zs)?; }
    let serial_prec = serial_prec_start.elapsed().as_secs_f64();

    let parallel_prec_start = Instant::now();
    for _ in 0..args.kernel_repeats { parallel_preconditioner.apply(&b, &mut zp)?; }
    let parallel_prec = parallel_prec_start.elapsed().as_secs_f64();

    println!();
    println!("Preconditioner microbenchmark");
    println!("serial / apply     : {:.3} ms", ms_per(serial_prec, args.kernel_repeats));
    println!("parallel / apply   : {:.3} ms", ms_per(parallel_prec, args.kernel_repeats));
    println!("precond speedup    : {:.3}x", serial_prec / parallel_prec.max(f64::MIN_POSITIVE));
    ...
    let serial_out = pcg_with_workspace(&parallel_operator, &preconditioner, &b, &mut xs, options, &mut ws)?;
    ...
    let parallel_out = pcg_with_workspace(&parallel_operator, &parallel_preconditioner, &b, &mut xp, options, &mut wp)?;
    ...
    println!();
    println!("Comparison");
    println!("iteration ratio    : {:.3} (parallel prec / serial prec)", parallel_out.iterations as f64 / serial_out.iterations.max(1) as f64);
    println!("solve-time ratio   : {:.3} (parallel prec / serial prec)", parallel_solve / serial_solve.max(f64::MIN_POSITIVE));
