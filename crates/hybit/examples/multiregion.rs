...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "local factor DOFs : {} total / {} unique",
        r.local_factor_dofs, r.unique_local_factor_dofs
    );
    println!("largest region    : {}", r.largest_local_region);
    println!(
        "factor memory     : {:.3} KiB",
        r.local_factor_bytes as f64 / 1024.0
    );
    println!(
        "total setup/solve : {:.3} / {:.3} ms",
        r.setup_seconds * 1.0e3,
        r.solve_seconds * 1.0e3
    );
    Ok(())
}
