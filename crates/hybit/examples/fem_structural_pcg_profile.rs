...
use hybit::{
    ...
    recommend_rigid_body_aggregate_nodes, Csr32Matrix, HybitError, KrylovOutcome,
    LinearOperator, ParallelCsr32Operator, ParallelRigidBodyTwoLevelPreconditioner,
    PcgWorkspace, Preconditioner, RigidBodyAggregation,
...
fn norm2(x: &[f64]) -> f64 { dot_serial(x, x).sqrt() }
...
fn print_stage(label: &str, seconds: f64, calls: usize, wall: f64) {
    ...
    let per_call_ms = if calls == 0 { 0.0 } else { total_ms / calls as f64 };
    let share = if wall == 0.0 { 0.0 } else { 100.0 * seconds / wall };
    println!("{label:<24}: {:9.3} ms total  {:7.3} ms/call  {:5.1}%  ({} calls)", total_ms, per_call_ms, share, calls);
}

fn main() -> Result<(), Box<dyn Error>> {
    let args = Args::parse()?;
    println!("HyBIT {} PCG vector-kernel profile", env!("CARGO_PKG_VERSION"));
    ...
    println!("dimensions         : {} x {}", matrix_profile.nrows, matrix_profile.ncols);
    ...
    println!("parallel index     : {:.3} MiB", mib(parallel_preconditioner.index_storage_bytes()));
    ...
    println!("reported residual  : {:.6e}", profiled_out.final_residual / norm2(&b).max(f64::MIN_POSITIVE));
    println!("verified residual  : {:.6e}", profiled_verified);
    println!("profiled wall      : {:.3} ms", profiled_wall * 1.0e3);
    println!("observed / iter    : {:.3} ms", if profiled_out.iterations == 0 { 0.0 } else { profiled_wall * 1.0e3 / profiled_out.iterations as f64 });
    println!();
    println!("Actual nested PCG stage timings");
    print_stage("operator A*x", stages.operator_seconds, stages.operator_calls, profiled_wall);
    print_stage("preconditioner", stages.preconditioner_seconds, stages.preconditioner_calls, profiled_wall);
    print_stage("dot reductions", stages.dot_seconds, stages.dot_calls, profiled_wall);
    print_stage("norm reductions", stages.norm_seconds, stages.norm_calls, profiled_wall);
    print_stage("x/r fused update", stages.update_x_r_seconds, stages.update_x_r_calls, profiled_wall);
    print_stage("p update", stages.update_p_seconds, stages.update_p_calls, profiled_wall);
    print_stage("initial residual", stages.residual_init_seconds, 1, profiled_wall);
    ...
    println!("accounted total        : {:9.3} ms  {:5.1}%", accounted * 1.0e3, 100.0 * accounted / profiled_wall.max(f64::MIN_POSITIVE));
    println!("timer/control overhead : {:9.3} ms  {:5.1}%", unaccounted * 1.0e3, 100.0 * unaccounted / profiled_wall.max(f64::MIN_POSITIVE));

    println!();
    println!("Uninstrumented control");
    println!("status             : {:?}", control_out.status);
    println!("iterations         : {}", control_out.iterations);
    println!("reported residual  : {:.6e}", control_out.final_residual / norm2(&b).max(f64::MIN_POSITIVE));
    println!("verified residual  : {:.6e}", control_verified);
    println!("solve time         : {:.3} ms", control_wall * 1.0e3);
    println!("profile/control    : {:.3}x", profiled_wall / control_wall.max(f64::MIN_POSITIVE));
